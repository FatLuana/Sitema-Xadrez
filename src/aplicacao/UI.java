...
public class UI{
	...
	private static void printCapturedPieces(List<PecaXadrez> captured) {
		List<PecaXadrez> white = captured.stream().filter(x -> x.getColor() == Cor.BRANCO).collect(Collectors.toList());
		List<PecaXadrez> black = captured.stream().filter(x -> x.getColor() == Cor.PRETO).collect(Collectors.toList());
